    ...
    for rule, evidence in find_matches(rules, ctx):
        ...
        for k, v in rule.overrides.items():
            ...
            if isinstance(v, (int, float)) and not isinstance(v, bool):
                v = str(v)
            # Numeric rule overrides act as ceilings: apply only when the
            # source actually carries a value and that value exceeds the
            # limit. An absent key means there is nothing to cap, so the rule
            # value is written as-is. Non-numeric overrides always apply.
            if k in out:
                try:
                    if float(str(out[k])) <= float(v):
